...
Aldagai berriarekiko ekuazio diferentzialak definituko ditugu eta horretarako (\ref{eq:aldfl1}) ekuazioak, katearen erregela aplikatuz deribatuko ditugu,
\begin{align*}
&\frac{d}{dt}\left(\begin{array}{c}
                q(t)  \\
                v(t)  \\
\end{array}\right)=
 \frac{d}{dt} \left(\varphi_t(Q(t),V(t)) \right) \ \Rightarrow \\
&\frac{d}{dt}\left(\begin{array}{c}
                q(t)  \\
                v(t)  \\
\end{array}\right)= 
...
\frac{d}{dt}\left(\begin{array}{c}
                Q(t)  \\
                V(t)  \\
\end{array}\right)= \varphi'_t(Q(t),V(t))^{-1} \ g(t, \varphi_t(Q,V)) \ \Rightarrow \\
...
Bestalde fluxu Kepleriarrak, Hamiltondarra sinplektikoa izateagatik, honako propietatea betetzen du,
\begin{equation}
\label{eq:Ksymp}
...
\end{equation*}
eta $\triangledown \ R(t,Q,V)$ deribatuari katearen erregela aplikatuz, berdintza hau,  
\begin{equation}
J^{-1} (\varphi'_t(Q,V))^T \triangledown r=J^{-1} \triangledown \ R(t,Q,V).
\end{equation}
betetzen dela baieztatzen da.
...
(\ref{eq: gEzH}) modu eraginkorrean konputatzeko, deribazio automatikoaren prozedura aplikatuz  kalkulatuko dugu. $(Q,V)$ aldagaien integrazioa  hiru urratsetan egingo dugu,
\begin{enumerate}
\item Kepler-en fluxua aplikatu, aldagai zaharretara itzultzeko (KeplerFlowGen). Funtzio honek, gero erabiliko ditugun tarteko aldagaiak ere itzuliko ditu. 
\begin{align*}
(q,v)= \varphi_t(Q,V). 
\end{align*}
\item Perturbazioak kalkulatu (GFcn).
\begin{equation*}
g=GFcn(t,q,v)
\end{equation*}
\item Fluxuaren deribatua eta honako espresioa konputatu (KeplerFlowGFcnaux).
\begin{align*}
